...
const Layout: React.FC<LayoutProps> = ({ user, onLogout, onViewChange, children, activeView }) => {
      ...
      <nav className="bg-slate-950/80 backdrop-blur-md border-b border-slate-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            >
              <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black group-hover:scale-110 transition-transform shadow-lg shadow-indigo-900/20">
                Q
              </div>
              ...
              <div className="hidden md:flex items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      onClick={onLogout}
                      className="p-2.5 bg-slate-900 text-slate-500 hover:text-rose-400 hover:bg-rose-950/50 rounded-xl border border-slate-800 transition-all"
                    >
                      <LogOut size={20} />
                    </button>
                  </div>
        ...
        {isMobileMenuOpen && user && (
          <div className="md:hidden border-t border-slate-800 bg-slate-900 px-4 py-6 space-y-4">
             ...
             <button 
               onClick={onLogout}
               className="w-full text-left px-4 py-3 rounded-xl text-sm font-bold text-rose-400 hover:bg-rose-950/30"
             >
               Logout
             </button>
          </div>
