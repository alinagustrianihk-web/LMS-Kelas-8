    ...
    return (
      <div className="py-8 px-4 space-y-12 animate-in zoom-in-95 duration-500">
        ...
        <div className="text-center space-y-6">
          ...
          <div>
            <h3 className="text-4xl font-black uppercase tracking-tighter mb-2 text-white italic">
              {feedback.title}
            </h3>
            <p className="text-slate-400 font-bold text-sm tracking-wide max-w-md mx-auto">
              {feedback.sub}
            </p>
          </div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-[3rem] p-10 max-w-sm mx-auto shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-center gap-3">
                 <div className="flex-1 text-[10px] font-black text-slate-300 uppercase bg-slate-900 border border-slate-800 px-4 py-2 rounded-2xl">
                ...
                >
                  <div className="flex justify-between items-start gap-4 mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                        {i + 1}
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className={`p-4 rounded-2xl border ${isCorrect ? 'bg-emerald-950/30 border-emerald-500/20' : 'bg-rose-950/30 border-rose-500/20'}`}>
                      ...
                      <p className={`text-xs font-black ${isCorrect ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {q.type === 'mcq' ? q.a?.[answers[i] as number] : (answers[i] ? 'True' : 'False')}
                      </p>
                    </div>
                    {!isCorrect && (
                      <div className="p-4 rounded-2xl border bg-emerald-950/20 border-emerald-500/20">
                        ...
                        <p className="text-xs font-black text-emerald-400">
                          {q.type === 'mcq' ? q.a?.[q.correct as number] : (q.correct ? 'True' : 'False')}
                        </p>
                      </div>
                    )}
                  </div>
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
      ...
      <div className="space-y-3">
        ...
        <div className="h-2.5 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-800">
           <div 
             className="h-full bg-gradient-to-r from-indigo-500 to-indigo-700 transition-all duration-700 shadow-[0_0_15px_rgba(79,70,229,0.3)]" 
             style={{ width: `${progress}%` }} 
           />
        </div>
      </div>

      <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800 shadow-sm relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:rotate-12 transition-transform duration-700">
          <Award size={120} className="text-white"/>
        </div>
        <div className="relative z-10">
          <h4 className="text-xl font-extrabold text-white leading-tight mb-10">
            {current.q}
          </h4>
            ...
            {current.type === 'mcq' ? (
              current.a?.map((opt, i) => (
                <button 
                  ...
                  className="group relative p-6 bg-slate-950 border-2 border-slate-800 rounded-3xl text-left font-bold text-slate-300 hover:border-indigo-600 hover:bg-indigo-900/20 hover:shadow-lg transition-all active:scale-[0.98]"
                >
