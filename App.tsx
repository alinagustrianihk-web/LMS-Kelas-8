...
import { 
  Trophy, CheckCircle2, Users, Eye, 
  UserPlus, AlertCircle, ChevronLeft, ArrowRight, 
  ShieldAlert, Trash2, Loader2, UserCircle2, 
  Mail, Lock, Key, Bot, Flame, Award, Clock,
  Search, SlidersHorizontal, ArrowUpDown, Filter,
  TrendingUp, BookOpen, Settings2, Plus, 
  EyeOff, MessageSquare, Gift, Megaphone,
  Sparkles, Image as ImageIcon, Map, Layers, User as UserIcon,
  Globe, LockIcon, ChevronRight
} from 'lucide-react';
...
const ContentRenderer: React.FC<{ content: Quest['content'] }> = ({ content }) => (
  <div className="space-y-8">
    {content.map((item, idx) => {
      if (item.type === 'h1') return <h1 key={idx} className="text-4xl font-black text-white border-l-8 border-indigo-600 pl-6 uppercase">{item.text}</h1>;
      if (item.type === 'h2') return <h2 key={idx} className="text-xl font-bold text-indigo-400 uppercase flex items-center gap-3"><div className="w-8 h-[2px] bg-indigo-900" />{item.text}</h2>;
      if (item.type === 'p') return <p key={idx} className="text-slate-300 font-medium leading-relaxed text-lg">{item.text}</p>;
        ...
        <div className="w-full md:w-80 space-y-6">
          <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800 shadow-xl shadow-indigo-900/10 space-y-6">
            <div className="text-center">
              <h2 className="text-5xl font-black text-white italic">{user.xp} <span className="text-xs text-indigo-400 uppercase tracking-widest block not-italic mt-1">Total XP Earned</span></h2>
            </div>
              ...
              return (
                <button
                  ...
                  className={`w-full p-5 rounded-[2rem] border transition-all text-left flex items-center gap-4 group ${
                    active ? 'bg-indigo-600 border-indigo-500 text-white shadow-xl shadow-indigo-900/20' : 
                    unlocked ? 'bg-slate-900 border-slate-800 text-slate-300 hover:border-indigo-600/50' : 
                    'bg-slate-950 border-slate-900 text-slate-600 cursor-not-allowed grayscale'
                  }`}
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${active ? 'bg-indigo-700 border-indigo-400' : 'bg-slate-950 border-slate-800'}`}>
                    {unlocked ? <Map size={18} /> : <Lock size={18} />}
                  </div>
                  <div>
        ...
        <div className="flex-1 space-y-8">
          <div className="bg-slate-900 p-10 rounded-[3.5rem] border border-slate-800 relative overflow-hidden">
            ...
            <div className="relative">
              <h2 className="text-4xl font-black uppercase text-white italic mb-2">
                {CHAPTERS.find(c => c.id === activeChapterId)?.title}
              </h2>
              <p className="text-slate-400 font-bold max-w-xl">
                {CHAPTERS.find(c => c.id === activeChapterId)?.description}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {currentChapterQuests.length > 0 ? currentChapterQuests.map((lvl: any, idx: number) => {
              ...
              return (
                <div 
                  ...
                  className={`relative h-64 rounded-[3rem] overflow-hidden cursor-pointer group border-4 transition-all ${
                    ...
                    isLocked ? 'border-slate-900 opacity-40 grayscale pointer-events-none' : 
                    'border-slate-800 hover:border-indigo-600 shadow-xl'
                  }`}
                >
                  ...
                  <div className="absolute inset-0 p-8 flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      ...
                      {isDone && <div className="bg-emerald-500 p-2 rounded-xl text-white shadow-lg"><CheckCircle2 size={16}/></div>}
                    </div>
                    <div>
...
const TeacherDashboard = ({ dbUsers, dbProgress, dbLevels, onUpdateQuest, onUpdateConfig, onRewardUser, onGenerateImage, systemConfig, showAlert }: any) => {
  ...
  const handleBulkGenerate = async (chapter: Chapter) => {
    if (!confirm(`Generate ${chapter.totalQuests} AI Quests for ${chapter.title}? (Each quest will have 5 questions)`)) return;
    setGeneratingChapter(chapter.id);
    try {
      const quests = await generateChapterQuests(chapter.id, chapter.title, chapter.totalQuests);
      for (let i = 0; i < quests.length; i++) {
        ...
        const fullQuest: Quest = {
          ...
          order: dbLevels.length + i + 1,
          title: q.title || `Quest ${i+1}`,
          topic: q.topic || 'English Mastery',
          passingScore: 80,
          rewardPoints: q.rewardPoints || 150,
          imageUrl: '',
          content: q.content || [],
          questions: (q.questions || []).slice(0, 5),
          status: 'draft'
        };
  ...
  if (managedChapterId) {
    ...
    return (
      <div className="max-w-7xl mx-auto py-12 px-6 space-y-8 animate-in slide-in-from-right-10 duration-500">
        <button 
          onClick={() => setManagedChapterId(null)}
          ...
          {quests.map((q: any) => (
            <div key={q.id} className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 flex items-center justify-between group hover:border-slate-700 transition-all">
              <div className="flex items-center gap-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border font-black ${q.status === 'published' ? 'bg-emerald-950 border-emerald-500/30 text-emerald-500' : 'bg-slate-950 border-slate-800 text-slate-600'}`}>
                   {q.order}
                </div>
                <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button 
                  ...
                  className="p-3 bg-slate-950 text-slate-500 hover:text-rose-500 rounded-xl border border-slate-800 hover:border-rose-900/50 transition-all"
                >
                  <Trash2 size={16} />
                </button>
              </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6 space-y-12">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-5xl font-black uppercase italic text-white">Teacher <span className="text-indigo-600">Terminal</span></h2>
        ...
        <div className="flex gap-2 bg-slate-900 p-2 rounded-2xl border border-slate-800">
          {['students', 'chapters', 'settings'].map(t => (
            <button key={t} onClick={() => setActiveTab(t as any)} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{t}</button>
          ))}
        </div>
      </div>
        ...
        <div className="bg-slate-900 rounded-[3rem] border border-slate-800 p-10">
          <table className="w-full text-left">
            <thead className="text-[10px] uppercase text-slate-500 border-b border-slate-800">
              <tr><th className="pb-6">Student</th><th className="pb-6 text-center">XP</th><th className="pb-6 text-center">Progress</th><th className="pb-6 text-right">Actions</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              {dbUsers.filter((u: any) => u.role === UserRole.STUDENT).map((s: any) => (
                <tr key={s.id} className="hover:bg-slate-950 transition-colors">
                  <td className="py-6 flex items-center gap-4">
                    ...
                    <div><p className="text-slate-100 font-bold">{s.name}</p><p className="text-[10px] text-slate-500 uppercase">@{s.username}</p></div>
                  </td>
                  <td className="text-center text-indigo-400 font-black">{s.xp}</td>
                  <td className="text-center">
                    <div className="w-24 h-2 bg-slate-950 rounded-full mx-auto overflow-hidden"><div className="h-full bg-emerald-500" style={{width: '40%'}} /></div>
                  </td>
                  <td className="text-right">
                    <button onClick={() => onRewardUser(s.id, 100)} className="p-3 bg-slate-900 text-slate-500 hover:text-amber-500 rounded-xl border border-slate-800 transition-all"><Gift size={18}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              <div key={chap.id} className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-xl space-y-6 flex flex-col justify-between group hover:border-indigo-600/30 transition-all">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 bg-indigo-950 rounded-2xl flex items-center justify-center text-indigo-500 border border-indigo-900/50"><Layers size={24}/></div>
                  ...
                  <p className="text-xs text-slate-500 font-bold mt-2">{chapQuests.length} Quests created out of {chap.totalQuests} syllabus points.</p>
                </div>
      ...
      {activeTab === 'settings' && (
        <div className="max-w-xl mx-auto space-y-8">
          <div className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-xl space-y-6">
            ...
            <textarea value={announcement} onChange={e => setAnnouncement(e.target.value)} className="w-full h-40 p-6 bg-slate-950 border border-slate-800 rounded-3xl text-white outline-none focus:border-indigo-600 transition-all resize-none" placeholder="Message to students..." />
            <button onClick={() => onUpdateConfig({ announcement })} className="w-full py-5 bg-indigo-600 text-white rounded-3xl font-black uppercase text-xs">Update Announcement</button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
    ...
    try {
      ...
      if (fetchedQuests.length === 0) {
        for (const q of INITIAL_CURRICULUM) {
           // Fix: Explicitly type fullQ as Quest to prevent status property widening from 'published' to string
           const fullQ: Quest = { ...q, status: 'published' };
           await dbService.saveQuest(fullQ);
        }
        // Fix: Use 'as const' to ensure status is treated as a literal type 'published' rather than string
  ...
  const handleQuestComplete = useCallback(async (questId: string, score: number, points: number) => {
    if (!user) return;
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white">
      <div className="w-32 h-32 bg-indigo-600 rounded-[3rem] flex items-center justify-center shadow-2xl animate-pulse-slow"><span className="text-6xl font-black italic">Q</span></div>
      <p className="mt-8 text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">Initializing Syllabus...</p>
    </div>
  );

  return (
    <Layout user={user} onLogout={() => {setUser(null); setView('dashboard');}} onViewChange={setView} activeView={view}>
      {msg && <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] bg-slate-900 px-8 py-4 rounded-full border border-indigo-500/50 text-white font-black text-xs uppercase tracking-widest shadow-2xl animate-in slide-in-from-top-10">{msg}</div>}
          ...
          <div className="w-full max-w-md bg-slate-900 p-10 md:p-12 rounded-[4rem] border border-slate-800 shadow-2xl space-y-8 animate-in zoom-in-95">
            <div className="text-center">
              ...
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-2">
                {isRegistering ? 'Initialize Adventurer' : 'Adventurer Login'}
              </p>
            </div>
        ...
        <>
          {view === 'quest-detail' && activeQuest ? (
            <div className="max-w-5xl mx-auto py-12 px-6 pb-32">
              <button onClick={() => setView('dashboard')} className="flex items-center gap-3 text-[10px] font-black uppercase text-slate-500 hover:text-indigo-400 mb-8 transition-colors"><ChevronLeft size={18}/> Back to Map</button>
              <div className="bg-slate-900 rounded-[4rem] border border-slate-800 shadow-2xl overflow-hidden">
                <div className="h-96 relative">
                  ...
                  <div className="absolute bottom-12 left-12"><h3 className="text-5xl font-black uppercase text-white italic">{activeQuest.topic}</h3></div>
                </div>
                <div className="p-8 md:p-16">
                  {isQuizMode ? (
                    <QuizEngine questions={activeQuest.questions} passingScore={activeQuest.passingScore} onFinish={(s) => {
                    ...
                    <div className="space-y-12">
                      ...
                      <button onClick={() => setIsQuizMode(true)} className="w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black uppercase text-lg flex items-center justify-center gap-4 hover:bg-indigo-700 active:scale-95 transition-all">Start Quest Challenge <ArrowRight size={24}/></button>
                    </div>
                  )}
                </div>
              </div>
             ...
             <TeacherDashboard 
               dbUsers={dbUsers} dbProgress={dbProgress} dbLevels={dbLevels} 
               onUpdateConfig={async (u:any) => { await dbService.saveConfig({...systemConfig, ...u}); setSystemConfig({...systemConfig, ...u}); showAlert("Updated."); }}
               onRewardUser={async (uid:string, p:number) => { const t = dbUsers.find(u=>u.id===uid); if(t){ const up = {...t, xp: t.xp+p}; await dbService.saveUser(up); setDbUsers(prev=>prev.map(u=>u.id===uid?up:u)); showAlert("Rewarded!"); }}}
               systemConfig={systemConfig} showAlert={showAlert}
             />
          ) : (
             <StudentDashboard 
                user={user} dbProgress={dbProgress} dbLevels={dbLevels} dbUsers={dbUsers} 
                setIsAiChatOpen={setIsAiChatOpen} setActiveQuest={setActiveQuest} 
                setView={setView} setIsQuizMode={setIsQuizMode} systemConfig={systemConfig}
             />
          )}

          {user.role === UserRole.STUDENT && (
            <AISensei topic={activeQuest?.topic || "English"} isOpen={isAiChatOpen} setIsOpen={setIsAiChatOpen} />
          )}
        </>
      )}
    </Layout>
