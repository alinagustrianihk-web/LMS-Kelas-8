...
import { 
  Trophy, Star, CheckCircle2, Users, Play, Eye, 
  Settings, BarChart3, UserPlus, AlertCircle, 
  ChevronLeft, ArrowRight, ShieldAlert, Info, Trash2,
  Loader2, UserCircle2, Mail, Lock, ShieldCheck, Key,
  BookOpen, LayoutDashboard, Zap, Award, Bot, Flame
} from 'lucide-react';
import { INITIAL_CURRICULUM, DEFAULT_USERS } from './constants';
import { User, UserRole, Quest, Progress, SystemConfig } from './types';
        ...
        <ul key={idx} className="space-y-3 bg-slate-900/50 p-8 rounded-[2.5rem] border-2 border-slate-800">
          {item.items?.map((li, i) => (
            <li key={i} className="flex gap-4 text-slate-300 font-bold text-base items-start">
              <div className="w-6 h-6 flex-shrink-0 rounded-full bg-indigo-950 flex items-center justify-center text-indigo-400 text-xs border border-indigo-900/50">
                {i + 1}
              </div>
              <span>{li}</span>
            </li>
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-10 text-center">
        ...
        <div className="bg-slate-900 p-10 rounded-[3rem] max-w-xl border-2 border-slate-800 shadow-2xl space-y-6">
          <p className="text-slate-300 font-medium">Izin Database ditolak. Pastikan Firebase Rules di Console sudah diatur ke <b>Mode Test</b> agar siswa bisa mendaftar.</p>
          <div className="bg-slate-950 p-6 rounded-2xl font-mono text-left text-indigo-400 text-xs border border-indigo-900/50">
            ...
            <div className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-xl relative overflow-hidden group">
              ...
              <div className="relative">
                 <div className="flex justify-between items-start mb-8">
                   ...
                   <div className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black italic text-lg shadow-xl shadow-indigo-900/20">
                     {user.xp} XP
                   </div>
                 </div>
                 <div className="flex items-center gap-8">
                   <div className="flex-grow h-4 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                      <div 
                        className="h-full bg-gradient-to-r from-indigo-500 to-indigo-700 transition-all duration-1000 shadow-[0_0_15px_rgba(79,70,229,0.3)]" 
              ...
              <div className="relative">
                 ...
                 <p className="text-xs font-bold text-slate-400 leading-relaxed">
                   {user.streak && user.streak > 0 
                     ? `Luar biasa! Kamu sudah belajar selama ${user.streak} hari berturut-turut.` 
                     : "Selesaikan 1 quest hari ini untuk memulai streak!"}
                 </p>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 p-8 flex flex-col justify-between text-white">
                    <div className="flex justify-between items-start">
                      <span className="bg-slate-900/60 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/10">
                        {lvl.title}
                      </span>
                      {isDone && (
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800 shadow-xl">
            ...
            <div className="space-y-3">
              {dbUsers
                ...
                .slice(0, 10)
                .map((u, i) => (
                <div key={u.id} className={`flex justify-between items-center p-4 rounded-2xl border transition-all ${u.id === user.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-xl scale-105' : 'bg-slate-950 border-slate-800'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs ${i === 0 ? 'bg-amber-400 text-amber-900' : i === 1 ? 'bg-slate-300 text-slate-700' : i === 2 ? 'bg-amber-700 text-amber-50' : 'bg-slate-800 text-slate-400'}`}>
                      {i + 1}
                    </div>
                    <div>
          ...
          <div className="bg-gradient-to-br from-indigo-700 to-violet-800 p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
             ...
             <button 
               ...
               className="bg-white text-indigo-700 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-colors shadow-lg"
             >
               Mulai Chat
             </button>
          </div>
    ...
    <div className="max-w-7xl mx-auto py-12 px-6 space-y-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 bg-slate-900 rounded-[3rem] border border-slate-800 shadow-xl p-10">
           <h3 className="font-black uppercase text-sm tracking-widest mb-8 flex items-center gap-3 text-slate-100"><Users size={20} className="text-indigo-500"/> Daftar Siswa</h3>
           <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left text-sm font-bold">
                 ...
                 <tbody className="divide-y divide-slate-800">
                    {dbUsers.filter(u => u.role === UserRole.STUDENT).sort((a,b) => b.xp - a.xp).map(s => {
                       ...
                       return (
                          <tr key={s.id} className="group hover:bg-slate-950 transition-colors">
                             ...
                             <td className="py-6 text-right">
                                <button className="p-2.5 text-slate-600 hover:text-indigo-500 transition-colors"><Eye size={18}/></button>
                             </td>
                          </tr>
                       );
                    })}
  ...
  const AdminPanel = () => (
    <div className="max-w-7xl mx-auto py-12 px-6 space-y-12 animate-in fade-in duration-700">
       <div className="flex justify-between items-center">
          <div>
            <h2 className="text-6xl font-black uppercase tracking-tighter italic text-white">Admin <span className="text-indigo-600">Core</span></h2>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-1">
             <form onSubmit={handleCreateUserByAdmin} className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-xl space-y-6">
                <h3 className="font-black uppercase text-sm flex items-center gap-3 text-slate-100"><UserPlus className="text-indigo-500"/> Daftarkan User</h3>
                  ...
                  <tbody className="divide-y divide-slate-800">
                     {dbUsers.map(u => (
                        <tr key={u.id} className="hover:bg-slate-950 transition-colors">
                           ...
                           <td className="p-6">
                              <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${u.role === UserRole.ADMIN ? 'bg-rose-950 text-rose-400 border border-rose-900/50' : u.role === UserRole.TEACHER ? 'bg-amber-950 text-amber-400 border border-amber-900/50' : 'bg-indigo-950 text-indigo-400 border border-indigo-900/50'}`}>
                                {u.role}
                              </span>
                           </td>
      ...
      {!user ? (
          ...
          <div className="relative w-full max-w-md bg-slate-900/80 backdrop-blur-xl p-12 rounded-[4rem] border border-slate-800 shadow-2xl space-y-8 animate-in zoom-in-95">
            <div className="text-center">
              ...
              <h2 className="text-3xl font-black uppercase tracking-tighter text-white italic">
                {authMode === 'login' ? 'Masuk Akademi' : 'Daftar Siswa'}
              </h2>
            ...
            {authMode === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-4">
                <div className="relative">
                  ...
                  <input name="email" type="email" placeholder="Email Sekolah" required className="w-full pl-14 pr-6 py-6 bg-slate-950 text-white rounded-[2rem] border-2 border-slate-800 focus:border-indigo-600 outline-none font-bold text-sm shadow-sm" />
                </div>
                <div className="relative">
                  <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                  <input name="password" type="password" placeholder="Password" required className="w-full pl-14 pr-6 py-6 bg-slate-950 text-white rounded-[2rem] border-2 border-slate-800 focus:border-indigo-600 outline-none font-bold text-sm shadow-sm" />
                </div>
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                <div className="relative">
                  ...
                  <input name="name" type="text" placeholder="Nama Lengkap" required className="w-full pl-14 pr-6 py-6 bg-slate-950 text-white rounded-[2rem] border-2 border-slate-800 focus:border-indigo-600 outline-none font-bold text-sm" />
                </div>
                <div className="relative">
                  <Mail className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                ...
                <div className="relative">
                  <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                  <input name="password" type="password" placeholder="Buat Password" required className="w-full pl-14 pr-6 py-6 bg-slate-950 text-white rounded-[2rem] border-2 border-slate-800 focus:border-indigo-600 outline-none font-bold text-sm" />
                </div>
        ...
        <>
          ...
          {user.role === UserRole.STUDENT && (
            <AISensei 
              topic={activeQuest?.topic || "General English Recount Text"} 
              isOpen={isAiChatOpen} 
